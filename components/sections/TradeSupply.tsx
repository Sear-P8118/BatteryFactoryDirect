...
export function TradeSupply() {
  return (
    <section className="py-20 lg:py-28 bg-zinc-100 border-t border-zinc-200" id="trade">
      <div className="max-w-screen-xl mx-auto px-6 lg:px-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">

          <AnimatedSection direction="left">
            <p className="text-red-600 text-xs font-semibold tracking-widest uppercase mb-4">Trade Supply</p>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link href="/quote"
                className="inline-flex items-center gap-2 border border-zinc-400 hover:border-zinc-600 text-zinc-700 hover:text-zinc-900 font-semibold py-3 px-6 rounded-lg transition-colors text-sm">
          ...
          <AnimatedSection direction="right" delay={0.1}>
            <div className="border border-zinc-200 rounded-xl overflow-hidden bg-white shadow-sm">
              ...
              <div className="divide-y divide-zinc-100">
                {types.map((t) => (
                  <div key={t} className="flex items-center justify-between px-6 py-4 hover:bg-zinc-50 transition-colors">
                    <span className="text-zinc-800 text-sm font-medium">{t}</span>
                    <span className="text-xs text-green-700 bg-green-100 px-2.5 py-0.5 rounded-full font-semibold">Eligible</span>
                  </div>
                ))}
              </div>
              <div className="bg-zinc-50 px-6 py-4 border-t border-zinc-200">
                <p className="text-zinc-400 text-xs">Contact us to set up a trade account and discuss your requirements.</p>
              </div>
            </div>
          </AnimatedSection>
        </div>
